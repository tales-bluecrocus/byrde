 ...
 */
function lakecity_maybe_render_editor(): void {
    ...
    if ( empty( $_GET['page'] ) || $_GET['page'] !== 'lakecity-editor' ) {
        return;
    }

    // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    $lakecity_page_id = isset( $_GET['lakecity_page_id'] ) ? absint( $_GET['lakecity_page_id'] ) : 0;

    // Fallback to old parameter name for backwards compatibility
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    if ( ! $lakecity_page_id && isset( $_GET['page_id'] ) ) {
        $lakecity_page_id = absint( $_GET['page_id'] );
    }

    if ( ! $lakecity_page_id || ! current_user_can( 'edit_post', $lakecity_page_id ) ) {
        wp_die( __( 'Invalid page or insufficient permissions.', 'lakecity' ) );
    }

    $page = get_post( $lakecity_page_id );
    if ( ! $page || $page->post_type !== 'page' ) {
        wp_die( __( 'Page not found.', 'lakecity' ) );
    }

    // Render full-screen editor and exit
    lakecity_render_editor_page( $lakecity_page_id, $page );
}
add_action( 'admin_init', 'lakecity_maybe_render_editor', 1 );

/**
 * Register the editor page (hidden from menu, required for URL to work)
 */
function lakecity_register_editor_page(): void {
    add_submenu_page(
        null, // Hidden from menu
        __( 'BlueCrocus Theme Editor', 'lakecity' ),
        __( 'BlueCrocus Theme Editor', 'lakecity' ),
        'edit_pages',
        'lakecity-editor',
...
add_action( 'admin_menu', 'lakecity_register_editor_page' );

/**
 * Render the full-screen editor page
 *
 * @param int     $lakecity_page_id The page ID.
 * @param WP_Post $page    The page post object.
 */
function lakecity_render_editor_page( int $lakecity_page_id, WP_Post $page ): void {
    $preview_url = add_query_arg( array(
        'lakecity_preview' => '1',
        'lakecity_page_id'          => $lakecity_page_id,
    ), home_url( '/' ) );

    $back_url = admin_url( 'edit.php?post_type=page' );
    $view_url = get_permalink( $lakecity_page_id );
    ...
    <head>
        ...
        <title><?php echo esc_html( sprintf( __( 'BlueCrocus Editor: %s', 'lakecity' ), $page->post_title ) ); ?></title>
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .lakecity-editor { display: flex; flex-direction: column; height: 100vh; background: #1e1e1e; }

            .lakecity-editor__header {
                display: flex;
                align-items: center;
            ...
            .lakecity-editor__left { display: flex; align-items: center; gap: 16px; }
            .lakecity-editor__right { display: flex; align-items: center; gap: 12px; }

            .lakecity-editor__back {
                display: flex;
                align-items: center;
                gap: 6px;
                color: #a7aaad;
            ...
            .lakecity-editor__back:hover { background: #32373c; color: #fff; }
            .lakecity-editor__back svg { width: 16px; height: 16px; }

            .lakecity-editor__title {
                color: #fff;
                font-size: 14px;
                font-weight: 500;
            }
            .lakecity-editor__title span { color: #a7aaad; font-weight: 400; }

            .lakecity-editor__btn {
                display: inline-flex;
                align-items: center;
                gap: 6px;
                padding: 8px 16px;
            ...
            .lakecity-editor__btn svg { width: 16px; height: 16px; }

            .lakecity-editor__btn--secondary {
                background: #32373c;
                color: #fff;
            }
            .lakecity-editor__btn--secondary:hover { background: #3c4349; }

            .lakecity-editor__btn--primary {
                background: #2271b1;
                color: #fff;
            }
            .lakecity-editor__btn--primary:hover { background: #135e96; }

            .lakecity-editor__iframe-wrap {
                flex: 1;
                position: relative;
                overflow: hidden;
            }

            .lakecity-editor__iframe {
                width: 100%;
                height: 100%;
                border: none;
            }

            .lakecity-editor__status {
                font-size: 12px;
                color: #a7aaad;
                padding: 4px 8px;
                border-radius: 3px;
            ...
            .lakecity-editor__status--saved { color: #68de7c; }
            .lakecity-editor__status--saving { color: #f0b849; }
        </style>
    </head>
    <body>
        <div class="lakecity-editor">
            <header class="lakecity-editor__header">
                <div class="lakecity-editor__left">
                    <a href="<?php echo esc_url( $back_url ); ?>" class="lakecity-editor__back">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                        Exit
                    </a>
                    <div class="lakecity-editor__title">
                        <?php echo esc_html( $page->post_title ); ?>
                        <span>#<?php echo esc_html( $lakecity_page_id ); ?></span>
                    </div>
                </div>
                <div class="lakecity-editor__right">
                    <span id="save-status" class="lakecity-editor__status">Ready</span>
                    <a href="<?php echo esc_url( $view_url ); ?>" target="_blank" class="lakecity-editor__btn lakecity-editor__btn--secondary">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        Preview
                    </a>
                </div>
            </header>
            <div class="lakecity-editor__iframe-wrap">
                <iframe
                    id="editor-iframe"
                    class="lakecity-editor__iframe"
        ...
        <script>
            ...
            window.addEventListener('message', function(e) {
                if (e.data && e.data.type === 'lakecity-save-status') {
                    const status = document.getElementById('save-status');
                    if (e.data.status === 'saving') {
                        status.textContent = 'Saving...';
                        status.className = 'lakecity-editor__status lakecity-editor__status--saving';
                    } else if (e.data.status === 'saved') {
                        status.textContent = 'Saved';
                        status.className = 'lakecity-editor__status lakecity-editor__status--saved';
                        setTimeout(() => {
                            status.textContent = 'Ready';
                            status.className = 'lakecity-editor__status';
                        }, 2000);
                    }
                }
            });
 ...
 */
function lakecity_add_editor_link( array $actions, WP_Post $post ): array {
    if ( $post->post_type !== 'page' || ! current_user_can( 'edit_post', $post->ID ) ) {
        return $actions;
    }

    $editor_url = admin_url( 'admin.php?page=lakecity-editor&lakecity_page_id=' . $post->ID );

    $actions['lakecity_editor'] = sprintf(
        '<a href="%s" style="color: #2271b1; font-weight: 500;">%s</a>',
        esc_url( $editor_url ),
        __( 'BlueCrocus Editor', 'lakecity' )
    );

    return $actions;
}
add_filter( 'page_row_actions', 'lakecity_add_editor_link', 10, 2 );

/**
 * Inject admin context into preview iframe
 */
function lakecity_inject_admin_context(): void {
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
add_action( 'wp_head', 'lakecity_inject_admin_context', 1 );

/**
 * Enqueue WordPress Media Library scripts for BlueCrocus Editor preview
 */
function lakecity_enqueue_media_library(): void {
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    if ( empty( $_GET['lakecity_preview'] ) ) {
        return;
    }
...
add_action( 'wp_enqueue_scripts', 'lakecity_enqueue_media_library' );

/**
 * Register REST API routes for page theme config
 */
function lakecity_register_theme_api_routes(): void {
    register_rest_route( 'lakecity/v1', '/pages/(?P<id>\d+)/theme', array(
        'methods'             => 'GET',
        'callback'            => 'lakecity_rest_get_page_theme',
    ...
    register_rest_route( 'lakecity/v1', '/pages/(?P<id>\d+)/theme', array(
        'methods'             => 'PUT',
        'callback'            => 'lakecity_rest_save_page_theme',
        'permission_callback' => function( WP_REST_Request $request ) {
            return current_user_can( 'edit_post', $request->get_param( 'id' ) );
        },
    ) );
    ...
    register_rest_route( 'lakecity/v1', '/upload-image', array(
        'methods'             => 'POST',
        'callback'            => 'lakecity_rest_upload_image',
        'permission_callback' => function() {
            return current_user_can( 'upload_files' );
        },
    ) );
}
add_action( 'rest_api_init', 'lakecity_register_theme_api_routes' );
 ...
 */
function lakecity_rest_get_page_theme( WP_REST_Request $request ): WP_REST_Response {
    $lakecity_page_id = $request->get_param( 'id' );
    $config  = get_post_meta( $lakecity_page_id, '_lakecity_theme_config', true );
 ...
 */
function lakecity_load_page_theme_config(): void {
    if ( ! empty( $_GET['lakecity_preview'] ) ) {
        return;
    }
    ...
    $lakecity_page_id      = get_the_ID();
    $theme_config = get_post_meta( $lakecity_page_id, '_lakecity_theme_config', true );
    ...
    if ( $config_data ) {
        ?>
        <script>
            window.lakecityConfig = <?php echo wp_json_encode( $config_data ); ?>;
        </script>
        <?php
    }
}
add_action( 'wp_head', 'lakecity_load_page_theme_config', 1 );

/**
 * Register meta key
 */
function lakecity_register_theme_meta(): void {
    register_post_meta( 'page', '_lakecity_theme_config', array(
        'type'         => 'string',
        'single'       => true,
        'show_in_rest' => false,
    ) );
}
add_action( 'init', 'lakecity_register_theme_meta' );
 ...
 */
function lakecity_customize_admin_bar( WP_Admin_Bar $wp_admin_bar ): void {
    ...
    $editor_url = admin_url( 'admin.php?page=lakecity-editor&lakecity_page_id=' . $page_id );

    $wp_admin_bar->add_node( array(
        'id'    => 'bluecrocus-editor',
        'title' => '<span class="ab-icon dashicons dashicons-edit" style="font-family: dashicons; font-size: 20px; line-height: 1; margin-right: 6px;"></span>' . __( 'BlueCrocus Editor', 'lakecity' ),
        'href'  => $editor_url,
        'meta'  => array(
            'title' => __( 'Edit with BlueCrocus Theme Editor', 'lakecity' ),
        ),
    ) );
}
add_action( 'admin_bar_menu', 'lakecity_customize_admin_bar', 81 );
