...
export default function FeaturedTestimonial() {
  ...
  const content = getContent('featured-testimonial');
  const theme = sectionThemes['featured-testimonial'] || {};
    ...
    <div className={`section-padding relative overflow-hidden ${hasBgImage ? '' : 'section-bg-primary'}`}>
      ...
      <div className="relative max-w-4xl mx-auto px-8 text-center">
        ...
        <div className="flex justify-center gap-1 mb-8">
          {[1, 2, 3, 4, 5].map((star) => (
            <StarIcon key={star} filled={star <= content.rating} />
          ))}
        </div>
        ...
        <div className="flex flex-col items-center">
          <div className="relative mb-4">
            ...
            <div className="relative w-20 h-20 rounded-full overflow-hidden border-4 section-bg-tertiary flex items-center justify-center" style={{ borderColor: 'var(--section-bg-primary, var(--color-dark-900))' }}>
              <span className="text-2xl font-bold section-text-primary">{getInitials(content.authorName)}</span>
            </div>
          </div>
        ...
        <div className="mt-12">
          ...
          >
            <LucideIcon name="phone" className="w-5 h-5 shrink-0" />
