...
const areas = [
  { name: "Coeur d'Alene", state: 'ID', featured: true },
  { name: 'Spokane', state: 'WA', featured: true },
  { name: 'Spokane Valley', state: 'WA', featured: true },
  { name: 'Post Falls', state: 'ID' },
  { name: 'Hayden', state: 'ID' },
  { name: 'Rathdrum', state: 'ID' },
  { name: 'Sandpoint', state: 'ID' },
  { name: 'Harrison', state: 'ID' },
  { name: 'Athol', state: 'ID' },
];

export default function ServiceAreas() {
  return (
    <div id="areas" className="py-24 section-bg-primary relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="font-[var(--font-display)] text-3xl sm:text-4xl lg:text-5xl font-bold section-text-primary mb-6">
            Serving <span className="text-primary-500">North Idaho & Spokane</span>
          </h2>
          <p className="section-text-secondary text-lg max-w-2xl mx-auto">
            Serving communities from North Idaho to the Spokane Valley with comprehensive services done by a trusted team
          </p>
        </div>
            ...
            <div className="flex flex-wrap gap-3">
              {areas.map((area, index) => (
                ...
                >
                  <MapPinIcon />
                  <span>{area.name}</span>
                  <span className={`text-xs ${area.featured ? 'text-white/80' : 'section-text-secondary opacity-70'}`}>
                    {area.state}
                  </span>
                </a>
              ))}
