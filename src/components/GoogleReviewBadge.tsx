...
const StarIcon = ({ type = 'full', size = 'sm' }: { type?: StarType; size?: 'sm' | 'md' }) => {
...
export default function GoogleReviewBadge({
  ...
  theme = 'light',
  showGoogleIcon = true,
  showReviewCount: _showReviewCount = false,
  className = '',
}: GoogleReviewBadgeProps) {
  const { globalConfig } = useGlobalConfig();
  const { rating, reviewCount } = globalConfig.googleReview;
  ...
  if (variant === 'full') {
    return (
      <div className={`flex items-center gap-2 ${className}`}>
        <span className="text-3xl font-bold section-text-primary">{rating}</span>
  ...
  return (
    <div
      className={`flex items-center gap-1.5 sm:gap-2 backdrop-blur-sm rounded-full px-3 sm:px-4 py-2 sm:py-2.5 shadow-sm ${className}`}
      style={{
    ...
    >
      ...
      {showGoogleIcon && (
        <>
          <div
            className="w-px h-3 sm:h-4 mx-0.5 sm:mx-1"
