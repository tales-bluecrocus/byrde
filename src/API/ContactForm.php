...
class ContactForm {
    ...
    public function register(): void {
        add_action( 'init', [ $this, 'register_leads_cpt' ] );
        add_filter( 'manage_byrde_lead_posts_columns', [ $this, 'leads_columns' ] );
        add_action( 'manage_byrde_lead_posts_custom_column', [ $this, 'leads_column_content' ], 10, 2 );
        add_filter( 'manage_edit-byrde_lead_sortable_columns', [ $this, 'leads_sortable_columns' ] );
        add_action( 'add_meta_boxes', [ $this, 'add_lead_meta_box' ] );
    ...
    /**
     * Register the byrde_lead CPT.
     */
    public function register_leads_cpt(): void {
        register_post_type( 'byrde_lead', [
            'labels'        => [
                'name'               => 'Leads',
                'singular_name'      => 'Lead',
                'menu_name'          => 'Leads',
    ...
    /**
     * Define custom columns for leads list table.
     */
    public function leads_columns( array $columns ): array {
        $new_columns = [];
    ...
    /**
     * Render custom column content for leads.
     */
    public function leads_column_content( string $column, int $post_id ): void {
        switch ( $column ) {
            case 'email':
            ...
            case 'phone':
                ...
                if ( $phone ) {
                    $raw = Manager::phone_to_raw( $phone );
    ...
    /**
     * Make service column sortable.
     */
    public function leads_sortable_columns( array $columns ): array {
     ...
     */
    public function add_lead_meta_box(): void {
        add_meta_box(
            'byrde_lead_details',
            'Lead Details',
            [ $this, 'render_lead_meta_box' ],
            'byrde_lead',
            'normal',
            'high'
        );
    ...
    public function render_lead_meta_box( \WP_Post $post ): void {
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td>
                    <?php if ( $phone ) : ?>
                        <a href="tel:<?php echo esc_attr( Manager::phone_to_raw( $phone ) ); ?>"><?php echo esc_html( $phone ); ?></a>
                    <?php endif; ?>
                </td>
            </tr>
            <tr>
    ...
    public function register_endpoint(): void {
        register_rest_route( Constants::REST_NAMESPACE, '/contact', [
            'methods'             => 'POST',
     ...
     */
    public function handle_form( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
        ...
        if ( is_array( $attribution ) && ! empty( $attribution ) ) {
            $allowed_keys = [ 'utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term', 'gclid', 'fbclid', 'msclkid', 'landing_page', 'referrer' ];
            $clean_attribution = [];
    ...
    /**
     * Check IP-based rate limit for contact form.
     */
    private function contact_rate_limit( string $ip, int $max_requests = 3, int $window = 300 ): bool {
    ...
    /**
     * Send lead notification email via Postmark.
     */
    ...
    /**
     * Build HTML email body for a lead notification.
     */
    private function build_lead_email_html( string $name, string $email, string $phone, string $service, string $message ): string {
        $phone_raw = Manager::phone_to_raw( $phone );
