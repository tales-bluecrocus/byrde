...
class TemplateLoader {
     ...
     */
    private function is_core_asset( string $handle, $registry ): bool {
     ...
     */
    private function is_allowed_handle( string $handle, string $type, bool $is_preview ): bool {
        $allowed = [ 'byrde-main', 'admin-bar', 'hoverintent-js' ];
        if ( 'style' === $type ) {
            $allowed[] = 'dashicons';
        }
        ...
        if ( $is_preview ) {
            ...
            return $this->is_core_asset( $handle, $registry );
        }
    ...
    public function isolate_assets(): void {
        ...
        if ( $wp_styles ) {
            foreach ( $wp_styles->queue as $handle ) {
                if ( ! $this->is_allowed_handle( $handle, 'style', $is_preview ) ) {
        ...
        if ( $wp_scripts ) {
            foreach ( $wp_scripts->queue as $handle ) {
                if ( ! $this->is_allowed_handle( $handle, 'script', $is_preview ) ) {
        ...
        add_filter( 'style_loader_tag', [ $this, 'filter_style_tag' ], 999, 2 );
        add_filter( 'script_loader_tag', [ $this, 'filter_script_tag' ], 999, 2 );
    }
     ...
     */
    public function filter_style_tag( string $tag, string $handle ): string {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        $is_preview = ! empty( $_GET[ Constants::QUERY_PREVIEW ] );
